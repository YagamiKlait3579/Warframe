...
;;;;;;;;;; Setting ;;;;;;;;;;
    /*
        Скрипт для атоматического применения-обновления способностей Dante в Warframe.

        Скрипт автоматически применяет выбранные комбинации способностей Dante:
        • Triumph — получение Overguard.
        • Wordwarden — призыв книги-саппорта.
        • Pageflight — применение способности для наложения дебаффа на врагов.

        Для каждой способности можно отдельно включить или отключить автоматическое применение.
        При включённом Auto-режиме скрипт повторяет выбранные способности через заданный интервал,
        поддерживая их активными без необходимости вручную нажимать способности.

        Также доступен Semi-режим, при котором выбранные комбинации применяются только один раз.
        Это позволяет использовать макрос как для постоянного поддержания
        бафов, так и для однократного применения нужных комбинаций.

        Время между повторениями можно изменять прямо во время работы скрипта.
        Текущее состояние скрипта, выбранные способности, режим работы и интервал повторения
        отображаются в интерфейсе на экране.

        Макрос запускается стандартной клавишей StartKey, указанной в файле Settings.ahk.
        
        Основные настройки находятся в секции Setting данного файла.
        ...
        Подробнее об их функционале вы можете прочитать на странице GitHub проекта.
    */
