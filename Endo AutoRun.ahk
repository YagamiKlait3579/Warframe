...
;;;;;;;;;; Setting ;;;;;;;;;;
/*
    Макрос для автоматического фарма Эндо на арене Vodyanoi (Седна).

    Макрос автоматизирует основные действия во время фарма: за Нидуса выполняет
    необходимую последовательность действий для сбора противников, а за Некроса
    автоматически перемещается в одну из двух позиций, выполняет необходимые
    действия и в конце собирает выпавшее Эндо. После завершения миссии макрос
    может автоматически запустить следующий забег.

    Для Некроса доступны два маршрута:
    • Left  — перемещение и сбор Эндо с левой позиции.
    • Right — перемещение и сбор Эндо с правой позиции.

    Также доступен отдельный режим Nidus, который можно запускать независимо
    для выполнения действий Нидуса. Дополнительно в макросе есть тестовые
    клавиши для проверки отдельных этапов маршрута без запуска полного цикла.

    ВАЖНО: этот макрос требует индивидуальной настройки. Его работа сильно
    зависит от характеристик компьютера, FPS, настроек игры, чувствительности
    мыши, скорости передвижения персонажа и используемого билда. Поэтому
    стандартные значения, скорее всего, не будут работать корректно у всех
    пользователей.

    Основные настройки находятся в разделе Setting ниже. Однако из-за большого
    количества точных действий часть настроек специально находится непосредственно
    внутри функций Nidus(), Nekros_Start() и Nekros_End().

    Если вам необходимо адаптировать макрос под себя, желательно изменять только
    значения, рядом с которыми есть комментарии. Комментарии объясняют, какое
    действие выполняется в этот момент и за что отвечает конкретное значение:
    например, сколько времени ждать после прыжка, насколько повернуть камеру
    или сколько времени двигаться в определённом направлении. Таким образом,
    для настройки не требуется разбираться в программировании — достаточно
    последовательно изменять указанные значения и проверять результат в игре.

    Макрос запускается стандартной клавишей StartKey, указанной в Settings.ahk.
    Первое нажатие запускает выбранный режим, повторное нажатие перезагружает
    макрос. Переключение между режимами Nidus / Left / Right выполняется
    отдельными клавишами, указанными в разделе Setting.

    Автоматический повтор забегов и автоматический сбор Эндо после окончания
    миссии также можно включать или отключать в разделе Setting.

    Важно понимать, что я не занимаюсь фармом Эндо профессионально и отвечал
    в этом проекте преимущественно за техническую часть автоматизации.
    Последовательность действий и маршруты основаны на инструкциях и гайдах
    игроков с дружественного сервера Platinum Farm Corp.

    На сервере Platinum Farm Corp можно найти подробный гайд по фарму Эндо,
    гайды для других миссий, а также билды персонажей, на которых
    тестировался данный макрос:
    https://discord.gg/f9YDj7ReXj

    Также существуют общие настройки проекта в файле Settings.ahk. Если какая-либо
    настройка присутствует и в этом макросе, и в Settings.ahk, значение из этого
    макроса имеет приоритет над значением из Settings.ahk.
